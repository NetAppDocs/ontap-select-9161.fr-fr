...
|===
. Une fois le nouveau serveur de gestion ajouté, vous pouvez éventuellement cliquer sur image:icon_kebab.gif["Options"] et sélectionnez l'une des options suivantes :
+
...
.Étapes
. Recherchez la clé générée dans le  `.ssh/id_***.pub` déposer.
