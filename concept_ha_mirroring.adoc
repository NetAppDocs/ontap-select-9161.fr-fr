...
Un cluster à deux nœuds avec médiateur peut couvrir deux centres de données. Pour plus d'informations, consultez la section link:reference_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices["Bonnes pratiques pour la haute disponibilité étendue à deux nœuds (MetroCluster SDS)"] .
...
* ONTAP Select agrégat en miroir* image:DDHA_03.jpg["ONTAP Select agrégat en miroir"]
