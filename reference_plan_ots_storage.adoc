...
NOTE: Les informations sur la prise en charge du stockage externe sont décrites dans link:reference_plan_ots_vnas.html["Configuration ONTAP Select vNAS"] .
...
NOTE: Si un contrôleur RAID matériel est présent, consultez le link:concept_stor_concepts_chars.html["Stockage en profondeur"] section pour les exigences de configuration supplémentaires.
...
Il existe également des exigences spécifiques de configuration des progiciels. Voir le link:https://docs.netapp.com/us-en/ontap-select/kvm-host-configuration-and-preparation-checklist.html#prepare-linux-server["préparation du serveur Linux"] étape pour plus d'informations.
...
+
** # cat /sys/block/<périphérique>/file d'attente/rotational
+
...
* Créer un pool de stockage système Vous devez créer un pool de stockage pour les données système ONTAP Select . Vous devez attacher le pool de stockage dans le cadre de la configuration du nœud ONTAP Select .
...
Vous devez configurer manuellement les lecteurs NVMe avant de les utiliser. Voir link:task_chk_nvme_configure.html["Configurer un hôte pour utiliser des lecteurs NVMe"] pour plus d'informations.
